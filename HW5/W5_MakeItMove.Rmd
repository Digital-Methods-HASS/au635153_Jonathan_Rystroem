...
pacman::p_load(tidyverse, gganimate, gapminder, png, gifski, gghighlight)
```
...
```
I have followed the answer proposed [here](https://stackoverflow.com/a/14564102)
...
I want to figure out which Nordic country has had the biggest improvement in health since the 1950'ies 
```{r}
gapminder %>% 
  filter(country %in% c("Denmark", "Norway", "Sweden", "Finland", "Iceland")) %>% 
  ...
  theme_minimal() + 
  labs(
    title = "Finland overtakes Denmark in Health",
    subtitle = "Finland versus the rest of the Nordics",
    x = "Year", 
    y = "Life Expectancy"
    ) + 
  gghighlight(country %in% c("Finland", "Denmark"), use_direct_label = FALSE)
```
