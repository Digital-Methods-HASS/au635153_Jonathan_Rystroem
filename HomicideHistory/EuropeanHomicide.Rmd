...
```{r plot long term trend first try}
...
Western_Europe <- Western_Europe %>% 
  rename(homicides = `Homicide rate in Europe over long-term (per 100,000) (homicides per 100,000 people)`)
```
...
Let's try with a line plot instead
```{r task 2}
ggplot(data = Western_Europe) + 
...
```
We notice that it is exactly similar 
